...
function normalizeTask(value: unknown, courseIds: Set<string>): StudyTask | null {
  ...
  return {
    ...
    origin: value.origin === "generated" || value.origin === "manual"
      ? value.origin
      : Boolean(value.autoGenerated)
        ? "generated"
        : "manual",
    locked: Boolean(value.locked) || undefined,
    dirty: Boolean(value.dirty) || undefined,
    completedAt: typeof value.completedAt === "string" && !Number.isNaN(new Date(value.completedAt).getTime()) ? value.completedAt : undefined,
    studyDayDate: optionalIsoDate(value.studyDayDate),
